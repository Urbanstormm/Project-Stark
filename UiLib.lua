...
local UILIB_GLOBAL = "ProjectStarkUILib"

local function publishLibrary(library)
	if typeof(_G) == "table" then
		_G[UILIB_GLOBAL] = library
	end
	return library
end
...
local function uniqueGuiName()
	return "PS_" .. HttpService:GenerateGUID(false):gsub("-", ""):sub(1, 16)
end
...
function Lib:Window(text, accent)
    if self._activeGui and self._activeGui.Parent then
        pcall(function()
    ...
    Library.Name = uniqueGuiName()
    Library.ResetOnSpawn = false
    Library.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    Library.Parent = getGuiParent()
    protectGui(Library)
    Library = cloneref(Library)
    self._activeGui = Library
    ...
    local WindowTabs = {}
    function WindowTabs:SetVisible(visible)
        uiVisible = visible == true
        MainFrame.Position = uiVisible and originalPosition or UDim2.new(-3, 0, -3, 0)
    end

    function WindowTabs:ToggleVisible()
        self:SetVisible(not uiVisible)
    end

    function WindowTabs:BindToggleKey(keyCode)
    ...
    function WindowTabs:Destroy()
        ...
        if Lib._activeGui == Library then
            Lib._activeGui = nil
        end
    end

    WindowTabs.Gui = Library
    WindowTabs.Main = MainFrame
